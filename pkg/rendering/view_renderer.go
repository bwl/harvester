...
func (v *ViewRenderer) GetDimensions() (int, int) {
...
func (v *ViewRenderer) UnregisterAll() {
	for k := range v.layers {
		v.layers[k] = v.layers[k][:0]
	}
	v.MarkDirtyAll()
}

func (v *ViewRenderer) MarkDirty(x, y, w, h int) {
	v.dirty = append(v.dirty, Rect{X: x, Y: y, W: w, H: h})
}
func (v *ViewRenderer) MarkDirtyAll() { v.dirtyAll = true }
func (v *ViewRenderer) DirtyRegions() []Rect {
	if v.dirtyAll {
		return []Rect{{0, 0, v.width, v.height}}
	}
	return v.dirty
}
...
func (v *ViewRenderer) Render() string {
	var start time.Time
	if os.Getenv("VR_PROFILE") == "1" {
		start = time.Now()
	}
	v.matrix.Clear()
	var all []RenderableContent
	for _, slice := range v.layers {
...
func (v *ViewRenderer) matrixToString() string {
	...
	emitStyle := func(g Glyph) {
		...
		if g.Foreground != current.fg {
			if (g.Foreground == Color{}) {
				seq += reset
			} else {
				seq += fmt.Sprintf("\x1b[38;2;%d;%d;%dm", g.Foreground.R, g.Foreground.G, g.Foreground.B)
			}
			current.fg = g.Foreground
		}
		if g.Background != current.bg {
			if (g.Background == Color{}) {
				seq += reset
			} else {
				seq += fmt.Sprintf("\x1b[48;2;%d;%d;%dm", g.Background.R, g.Background.G, g.Background.B)
			}
	...
	if !start.IsZero() {
		fmt.Fprintf(os.Stderr, "[vr] stringify=%s\n", time.Since(start))
	}
	return string(b)
}
...
func (v *ViewRenderer) RenderPatch() []LinePatch {
	if v.dirtyAll {
		v.dirty = []Rect{{0, 0, v.width, v.height}}
	}
